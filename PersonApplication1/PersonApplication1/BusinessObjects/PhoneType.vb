...
#Region " Private Methods "
    Private Function Insert(database As DatabaseHelper.Database) As Boolean
        ...
        Try
            ...
            database.Command.CommandText = "tblPhoneType_INSERT"
    ...
    Private Function Update(database As DatabaseHelper.Database) As Boolean
        ...
        Try
            ...
            database.Command.Parameters.Clear()
            database.Command.CommandType = CommandType.StoredProcedure
            database.Command.CommandText = "tblPhoneType_UPDATE"
            'Add the header data parameters
    ...
    Private Function Delete(database As DatabaseHelper.Database) As Boolean
        ...
        Try
            database.Command.Parameters.Clear()
            database.Command.CommandType = CommandType.StoredProcedure
            database.Command.CommandText = "tblPhoneType_DELETE"
            MyBase.Initialize(database, MyBase.Id)
            database.ExecuteNonQuery()
            MyBase.Initialize(database.Command)
...
#Region " Public Methods "
    Public Function Save() As PhoneType
    ...
    Public Function GetById(id As Guid) As PhoneType
        ...
        db.Command.CommandText = "tblPhoneType_getById"
        ...
        Else
            If ds.Tables(0).Rows.Count = 0 Then
                Throw New Exception(String.Format("Phone Type {0} was not fount", id))
            Else
                Throw New Exception(String.Format("Phone Type {0} found multiple records", id))
            End If
        End If
